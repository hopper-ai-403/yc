...
class AudioAsset(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    technical_completed: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
    )
    technical_version: Mapped[str | None] = mapped_column(String(32), nullable=True)
    technical_completed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
    ...
    speech_completed: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
    )
    speech_completed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
